...
Windows 端已完成 Phase 1–2：除原生启动、LocalAppData 存储、单实例和自身重启外，服务监控页现在会展示真实 Windows TCP 监听、PID、命令行、工作目录、CPU/内存、运行时长和父进程来源。进程所有者用访问令牌 SID 校验；监控页可结束经验证的当前用户进程（Windows 上为立即终止）。受管应用的 Job Object 生命周期仍属于 Phase 3，Windows 原生选择器/脚本命令适配属于 Phase 4。macOS 现有能力不受影响。
...
- Windows 启动需要 PowerShell 5.1 或 PowerShell 7。
...
- **Windows 说明**：工作目录使用只读 NT 进程参数尽力获取，对受保护或无法访问的进程会留空；结束进程前会在同一句柄上重新校验 SID，防止 PID 复用。
