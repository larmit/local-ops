...
总控台会以当前操作系统用户的权限运行，并提供保存命令和管理本地进程的接口。请把命令执行、身份校验、写接口授权、路径处理、配置完整性和敏感信息泄露问题视为高影响安全问题。
...
- `%LOCALAPPDATA%\LocalOps\` 中的 Windows 配置、图标和日志；
...
Windows 服务监控只展示访问令牌 SID 与总控台当前用户一致的进程。结束操作使用已打开句柄上的 SID 二次校验，但 Windows 没有通用的 SIGTERM 等价物，因此服务表中的「结束进程」是立即终止。受管服务的优雅停止属于 Job Object 生命周期。
